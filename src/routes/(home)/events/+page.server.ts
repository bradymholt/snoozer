...
import { events, eventValidator, getEventAppearancesByContact } from "$lib/server/schema/events";
...
export const load: PageServerLoad = async ({ locals }) => {
  ...
  const recentEvents = await db.select().from(events).orderBy(desc(events.date)).limit(5);
  ...
  return {
    ...
    eventAppearancesByContact: await getEventAppearancesByContact(db, recentEvents, recentEventAppearances),
  };
};
...
export const actions = {
  saveEvent: async ({ request }) => {
    ...
    if (event.error) {
      ...
      await db.update(events).set(event.data).where(eq(events.id, event.data.id));
      // For simplicity, we delete all appearances and re-insert them
      await db.delete(appearances).where(eq(appearances.eventId, event.data.id));
